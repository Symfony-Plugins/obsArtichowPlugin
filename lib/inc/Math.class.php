...
class awPolygon extends awShape {
	...
	public function getLines() {
		...
		for($i = 0; $i < $count - 1; $i++) {
			$lines[] = new awLine($this->get($i), $this->get($i + 1));
		}
		
		// "Close" the polygon
		$lines[] = new awLine($this->get($count - 1), $this->get(0));
	...
	public function getBoxPoints() {
		...
		$upperLeft  = new awPoint(min($x), min($y));
		$lowerRight = new awPoint(max($x), max($y));
